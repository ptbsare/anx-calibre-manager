...
### 6. 为什么左侧 Calibre 书库没有删除书籍的按钮？

**这是刻意设计的安全特性。** 当 Anx Calibre Manager 暴露在公网时，为了防止误操作或恶意删除您书库中的书籍（特别是拥有成千上万本图书的书库），我们没有提供删除 Calibre 书籍的接口。

**如需删除 Calibre 书籍，请使用以下方式：**
- 通过内网访问 Calibre 内容服务器（默认端口 `8080`）进行删除操作
- 或使用 Calibre 桌面客户端进行管理

这样可以确保您的书库在享受 Anx Calibre Manager 便利功能的同时，不会因暴露在公网上而面临数据丢失的风险。

### 7. 我不想用笨重的 Calibre 客户端或简陋的 calibre-server 来管理书库，能用 Calibre-Web、Calibre-Web-Automated 或 Talebook 等前端吗？
